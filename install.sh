...
echo -e "${BLUE}                                    https://github.com/Croakieee/Automated-PowerHacker-Suite.git ${NC}" 
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        if sudo git clone https://github.com/Croakieee/Automated-PowerHacker-Suite.git $install_dir; then
